...
export default function StudentList() {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        const response = await getAllStudent({ limit: Number(limit), sortBy, orderBy, search, page });
    ...
    { accessor: 'phoneNumber', title: 'Phone Number' },
    {
      ...
      render: (row) => (
        <Group>
          <ActionIcon 
            color="blue" 
            onClick={() => router.push(`/student-update/${row.id}`)}
          >
            <IconEdit />
          </ActionIcon>
  ...
  const handleDeleteUser = async (userId) => {
    try {
      ...
      setStudent(users.filter((user) => user.id !== userId));
