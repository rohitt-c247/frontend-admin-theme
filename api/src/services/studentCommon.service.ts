...
import type { IStudent } from '@customTypes';
import { Student, studentModelToDomain } from '@models';
 ...
 */
export const getUserById = async (userId: string): Promise<IStudent | null> => {
 ...
 */
export const updateUserByEmail = async (email: string, updates: Partial<IStudent> | null) => {
  const userDoc: IStudent | null = await getUserByEmail(email);
    ...
    Object.keys(updates).forEach((key) => {
      if (userDoc[key] !== updates[key]) {
        ...
        if (key === 'preferredTwoFAMethods') {
          ...
          const currentMethodType = updates[key]?.[0] ? updates[key][0].methodType : undefined;
          if (
            currentMethodType &&
            userDoc.preferredTwoFAMethods &&
            !userDoc.preferredTwoFAMethods.some((method: any) => method.methodType === currentMethodType)
          ) {
            // If the method is not already in the array, push it
            if (userDoc[key]) {
              userDoc[key].push(...(updates[key] ?? []));
            } else {
  ...
  return studentModelToDomain(updatedUserDoc);
};
